...
class _CalculatorState extends State<Calculator> {
  ...
  /// 사용자가 입력한 계산식을 나타내는 문자열
  ///
  /// 사용자가 숫자나 연산자 버튼을 누를 때마다 해당 버튼의 텍스트가 `equation` 변수에 추가되고,
  /// 이를 기반으로 `expression`변수를 업데이트함
  ///
  /// `expression` 변수는 계산식을 math_expressions 패키지의 Parser 클래스에 의해 수학 표현식으로 변환됨
  String expression ="";
  ...
  String calculateResult() {
    String modifiedExpression = equation;
    // '%'를 '*0.01'로 치환하여 계산식에 포함시킴
    modifiedExpression = modifiedExpression.replaceAll('%', '*0.01');
